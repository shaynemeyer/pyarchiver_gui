...
class Items:
    ...
    def __init__(self, path, name, size, item_type, modified):
        self.path = path
        self.name = name
        self.size = size
    ...
    def on_create_archive(self, event):
        if not self.archive_olv.GetObjects():
            self.show_message("No files / folders to archive", "Error", wx.ICON_ERROR)
            return

        if not self.archive_filename.GetValue():
            self.show_message("File name is required!", "Error", wx.ICON_ERROR)
            return
...
class ArchivePanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        # Create input widget
        self.archive_olv = ObjectListView(self, style=wx.LC_REPORT | wx.SUNKEN_BORDER)
        ...
        self.archive_types = wx.ComboBox(
            self, value="Tar", choices=["Tar"], size=(75, -1)
        )
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                full_save_path = pathlib.Path(
                    ...
                    "{filename}.{type}".format(
                        filename=archive_filename, type=archive_type.lower()
                    ),
                )
                controller.create_archive(
                    full_save_path, self.archive_olv.GetObjects(), archive_type
                )
                message = f"Archive created at {full_save_path}"
                self.show_message(message, "Archive Created", wx.ICON_INFORMATION)
    ...
    def update_display(self, items):
        ...
        for path in paths:
            ...
            item = Items(path, basename, size, item_type, last_modified)
...
class MainFrame(wx.Frame):
    ...
    def __init__(self):
        """Constructor"""
        super().__init__(None, title="PyArchiver", size=(800, 600))
    ...
    def create_menu(self):
        ...
        exit_menu_item = file_menu.Append(wx.ID_ANY, "Exit", "Exit the application")
    ...
    def create_toolbar(self):
        ...
        add_file_tool = self.toolbar.AddTool(
            wx.ID_ANY, "Add File", add_ico, "Add a file to be archived"
        )
        self.Bind(wx.EVT_MENU, self.on_add_file, add_file_tool)

        add_folder_ico = wx.ArtProvider.GetBitmap(
            wx.ART_FOLDER_OPEN, wx.ART_TOOLBAR, (16, 16)
        )
        add_folder_tool = self.toolbar.AddTool(
            wx.ID_ANY, "Add Folder", add_folder_ico, "Add a folder to be archived"
        )
        self.Bind(wx.EVT_MENU, self.on_add_folder, add_folder_tool)

        remove_ico = wx.ArtProvider.GetBitmap(wx.ART_MINUS, wx.ART_TOOLBAR, (16, 16))
        remove_tool = self.toolbar.AddTool(
            wx.ID_ANY, "Remove", remove_ico, "Remove selected item"
        )
